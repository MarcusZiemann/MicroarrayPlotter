...
ui <-fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      tabsetPanel(
        tabPanel("Main",
                 selectInput("spec", "Please select organism:", 
                             choices =l),
                             #choices =c("Anabaena sp. PCC 7120", "Synechocystis sp. PCC 6803")),
                 prettySwitch("own_map", label = "use own Gene-map?", value= FALSE),
                 ...
                 fileInput("Name", "enter names of Microarrays", multiple = FALSE, accept = c(".txt")),
                 uiOutput("whichReplicons"),
                 numericInput("start", "Please enter the start of your read", 1, step = 1000),
                 numericInput("end", "Please enter the end of your read", 1e4, step = 1000),
